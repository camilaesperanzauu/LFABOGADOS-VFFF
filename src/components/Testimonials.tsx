...
const Testimonials = () => {
  return (
    <section className="py-20 bg-main-rose text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div key={index} className="bg-white p-8 rounded-lg shadow-md flex flex-col h-full">
              <p className="italic text-dark-text mb-8 text-lg">"{testimonial.text}"</p>
                              <div className="flex items-center border-t border-main-rose pt-4 mt-auto">
                  <div className="bg-main-rose p-2 rounded-full mr-3">
                    <User size={20} className="text-white" />
                  </div>
                  <span className="font-bold text-dark-text">{testimonial.name}</span>
                </div>
            </div>
          ))}
        </div>
